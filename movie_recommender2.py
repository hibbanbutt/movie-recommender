...
def main():
    ...
    # Read movie_processed dataset
    movie_processed = pd.read_parquet('movie_processed_cleanedd.parquet')
    movie_processed = movie_processed.rename(columns={'movie_id': 'MovieID'})
    ...
    # Merge movie_processed DataFrame with the movies DataFrame
    movies = pd.merge(movies, movie_processed[['MovieID', 'cleaned_description']], on='MovieID')
    movies = movies.rename(columns={'cleaned_description': 'Intro'})


    vectorizer = TfidfVectorizer(stop_words='english', max_features=1000)
    intro_matrix = vectorizer.fit_transform(movies['Intro'])

    # Vectorize genres and use these to cluster the movies
    feature_vector = intro_matrix
